...
#TODO специальный аргумнт, что бы контент ascii_char строк выводился полностью(та часть, которая следует после \0 может быть полезной)(затем посмотреть Rayne.skl файл ещё раз)
#TODO Возможно, по умолчанию добавлять расширение .json при записи в файл, спец. опция для отключения
